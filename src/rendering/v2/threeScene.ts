...
import { slicePanelFootprint } from "../../geometry/v2/wallNetwork";
import type {
  BalconyGeometryV2,
  HouseGeometryV2,
  SlabGeometryV2,
...
function projectBoundsV2(project: HouseProject): SceneBounds {
...
function refineBoundsMaxY(bounds: SceneBounds, geometry: HouseGeometryV2): SceneBounds {
...
function createWallMeshes(project: HouseProject, geometry: HouseGeometryV2) {
...
function createOpeningFrameMeshes(project: HouseProject, geometry: HouseGeometryV2) {
  const materials = new Map<string, THREE.MeshStandardMaterial>();
  const meshes: THREE.Mesh[] = [];
...
function buildSlabMesh(slab: SlabGeometryV2, material: THREE.Material): THREE.Mesh {
...
function createSlabMeshes(project: HouseProject, geometry: HouseGeometryV2) {
  const materials = new Map<string, THREE.MeshStandardMaterial>();
  const meshes: THREE.Mesh[] = [];
...
function createRoofMeshes(project: HouseProject, geometry: HouseGeometryV2) {
  const meshes: THREE.Mesh[] = [];
...
function createStairMeshes(project: HouseProject, geometry: HouseGeometryV2) {
  const materials = new Map<string, THREE.MeshStandardMaterial>();
  const meshes: THREE.Mesh[] = [];
...
function createBalconyMeshes(project: HouseProject, geometry: HouseGeometryV2) {
...
export function mountHouseSceneV2(
  ...
  const houseGeometry: HouseGeometryV2 = buildSceneGeometryV2(project);

  let bounds = projectBoundsV2(project);
