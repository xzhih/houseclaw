...
export function projectElevation(
  ...
  const wallBands: ElevationWallBand[] = visibleWalls.map((wall) => {
  ...
  const openings: ElevationOpeningRect[] = project.openings
  ...
  const balconies: ElevationBalconyRect[] = project.balconies
  ...
  const roofPolygons: ElevationRoofPolygon[] = [];
